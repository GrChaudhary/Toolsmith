...
test('style selection synchronizes page, picker, and canonical SVG without touching geometry', () => {
  ...
  assert.match(runtime, /\['classic', 'signal-flow', 'blueprint', 'editorial', 'contrast'\]/);
...
test('same topology keeps identical canonical SVG geometry across all five styles', () => {
  const normalize = (svg) => svg.replace(/ data-preset="(?:classic|signal-flow|blueprint|editorial|contrast)"/, '');
  const variants = ['classic', 'signal-flow', 'blueprint', 'editorial', 'contrast'].map((preset) => normalize(svgBlock(render('architecture', preset))));
